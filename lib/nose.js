...
Nose.prototype.loadFiles = function loadFiles (fn) {
    var self = this
      , suite = this.suite
...
function buildTree (node, parent) {
    ...
    if (!(nodeId in __nodesRegister)) {
        __nodesRegister[nodeId] = new TreeNode(node);
    }

    if (!(parentId in __nodesRegister)) {
        __nodesRegister[parentId] = new TreeNode(parent);
    }

    __nodesRegister[parentId].addChild(__nodesRegister[nodeId]); // add child to parent's children
    __nodesRegister[nodeId].setParentNode(__nodesRegister[parentId]); // mark child as parent's child

    if (!parent) {
        __tree.addChild(__nodesRegister[nodeId]);
    }
}
