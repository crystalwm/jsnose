...
Nose.prototype.run = function run (callback) {
    ...
    var detector = new exports.Detector();
    // var reporter = new this._reporter(detector, options);
    ...
    this.files.forEach(function (file) {
        ...
        try {
            ...
            traverseDown(__tree, analyzer.run);
...
function buildTree (node, parent) {
    ...
    if (!(__nodeId in __nodesRegister)) {
        __nodesRegister[__nodeId] = new TreeNode(node);
    }

    if (!(__parentId in __nodesRegister)) {
        __nodesRegister[__parentId] = new TreeNode(parent);
    }
